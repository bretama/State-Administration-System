...
class KetemaReport extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($woredas as $woreda) {
            ...
            foreach ($tabias as $tabia) {
                ...
                if($tabia['isUrban'] == 'ከተማ'){
                    ...
                    ->where('zoneworedaCode', $zoneCode.$woreda['woredaCode'].$tabia['tabiaCode'])->count();
                    $cd = Hitsuy::where('hitsuys.occupation', 'ዘይመንግስታዊ') ->where('hitsuys.hitsuyID','LIKE', $zoneCode.$woreda['woredaCode'].$tabia['tabiaCode'].'/%')->where('hitsuy_status', 'ሕፁይ')->count();
                ...
                if($tabia['isUrban'] == 'ከተማ'){
                    ...
                    foreach (meseretawiWdabe::where('tabiaCode', $tabia['tabiaCode'])->where('type', 'መምህራን')->get()->toArray() as $widabe){
                        $row_wahio_count[2] += Wahio::where('widabeCode', $widabe['widabeCode'])->count();
                    }

                    // ሲ/ሰርቫንት
                    foreach (meseretawiWdabe::where('tabiaCode', $tabia['tabiaCode'])->where('type', 'ሲ/ሰርቫንት')->get()->toArray() as $widabe){
                        $row_wahio_count[3] += Wahio::where('widabeCode', $widabe['widabeCode'])->count();
                    }

                    // ሸቃሎ
                    foreach (meseretawiWdabe::where('tabiaCode', $tabia['tabiaCode'])->where('type', 'ሸቃሎ')->get()->toArray() as $widabe){
                        $row_wahio_count[4] += Wahio::where('widabeCode', $widabe['widabeCode'])->count();
                    }
                ...
                if($tabia['isUrban'] == 'ከተማ'){
                    ...
                    // ግልጋሎት
                    $row_new_members_deant[10] += Hitsuy::where('hitsuyID','LIKE', $zoneCode.$woreda['woredaCode'].$tabia['tabiaCode'].'/%')->where('hitsuy_status', 'ሕፁይ')->where('occupation', 'ግልጋሎት')->count();
                }
        ...
        return view('report.ketema', compact('zoneName', 'weseking_gudletin', 'abalat_age_education', 'abalat_mahberawi_bota', 'abalat_deant', 'wahio_count', 'tabia_count', 'plan_deant', 'plan_non_deant', 'plan_all', 'model_members', 'new_members_non_deant', 'new_members_deant', 'approved_new_members', 'grades', 'punishment'));
    }
}
