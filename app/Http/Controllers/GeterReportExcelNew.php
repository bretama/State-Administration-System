...
class GeterReportExcelNew extends Controller
{
    private function loadData($zoneCode){
        ...
        return [$zoneName, $weseking_gudletin, $abalat_age_education, $abalat_mahberawi_bota, $wahio_count, $tabia_count, $plan_non_deant, $plan_all, $model_members, $new_members_non_deant, $approved_new_members, $grades, $punishment];
    }
    public function index(Request $request)
    {
        ...
        header('Content-Disposition: attachment;filename="' . $zoneName . '_' . explode("/", $today)[2] . '_' . $this_quarter . '_ናይ_ገጠር_ሪፖርት.xlsx"');
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('F'.($i+1), 'ዕድመ ኣባልነት')
            ->setCellValue('J'.($i+1), 'ኣብ ማሕበራት ዘሎ ኣባል')
            ->setCellValue('N'.($i+1), 'ብዘለዎ ስራሕ')
            ->setCellValue('Q'.($i+1), 'ብፆታ')
            ->setCellValue('B'.($i+2), 'ገባር')
            ->setCellValue('C'.($i+2), 'ካልኦት ሰብ ሞያ')
            ->setCellValue('D'.($i+2), 'መምህራን')
            ->setCellValue('E'.($i+2), 'ተምሃሮ')
            ->setCellValue('F'.($i+2), '67-83')
            ->setCellValue('G'.($i+2), '84-93')
            ->setCellValue('H'.($i+2), '94-2000')
            ->setCellValue('I'.($i+2), 'ድሕሪ 2001')
            ->setCellValue('J'.($i+2), 'ደ/ኣንስትዮ')
            ->setCellValue('K'.($i+2), 'ሓረስታይ')
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('K'.($i+1), 'ጠ/ድምር መ/ውዳበታት')
            ->setCellValue('C'.($i+2), 'ልዕሊ 500')
            ->setCellValue('D'.($i+2), '500ን ትሕቲኡ')
            ->setCellValue('E'.($i+2), 'ድምር')
            ->setCellValue('F'.($i+2), 'ገባር')
            ->setCellValue('G'.($i+2), 'ተምሃሮ')
            ->setCellValue('H'.($i+2), 'መምህራን')
            ->setCellValue('I'.($i+2), 'ካልኦት ሰብ ሞያ')
            ->setCellValue('J'.($i+2), 'ድምር')
            ->mergeCells('A'.$i.':P'.$i)
            ->mergeCells('A'.($i+1).':A'.($i+2))
            ->mergeCells('B'.($i+1).':B'.($i+2))
            ->mergeCells('C'.($i+1).':E'.($i+1))
            ->mergeCells('F'.($i+1).':J'.($i+1))
        ...
        $i += 2;
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A'.$i, 'ዞባ '.$data[0].': ውዳበታት ገጠር ትልሚ ምድላው')
            ->setCellValue('A'.($i+1), 'ወረዳ')
            ->setCellValue('B'.($i+1), 'ትልሚ ምድላው መ/ውዳበ ደኣንት')
            ->setCellValue('J'.($i+1), 'ድምር መ/ውዳበታት')
            ->setCellValue('B'.($i+2), 'ሓረስታይ')
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G'.($i+1), 'ካልኦት ሰብ ሞያ')
            ->setCellValue('L'.($i+1), 'መምህራን')
            ->setCellValue('Q'.($i+1), 'ተምሃሮ')
            ...
            ->setCellValue('R'.($i+2), 'ቶፕ 20')
            ->setCellValue('S'.($i+2), 'ማእኸላይ')
            ->setCellValue('T'.($i+2), 'ትሑት')
