        ...
        <div class="">
            ...
            <div class="table-responsive text-center">
                ...
                <thead>
                    <tr>
                        ...
                        <th class="text-center hidden">መ.ቑ መሰረታዊ ው</th>
            ...
            <tbody>
                        @foreach ($data as $mywahio)
                          <tr>
                            <td><input style="display: inline;" type="checkbox" class="checkbox" name="check[]" value="{{{ $mywahio->id }}}"><td>
                            <td class="hidden">{{ $mywahio->widabeCode }}</td>  
                            <td class="hidden">{{ $mwidabedata[$mywahio->widabeCode] }}</td>                              
                            <td class="hidden">{{ $tabiadata[$mwidabedata[$mywahio->widabeCode]] }}</td>  
                            <td class="hidden">{{ $woredadata[$tabiadata[$mwidabedata[$mywahio->widabeCode]]] }}</td>  
                            <td>{{ $mywahio->id }}</td>                                                      
                            <td>{{ $mywahio->wahioName }}</td>                            
                            <td>{{ $collectionyear[$mywahio->id] }}</td>                             
                          </tr>                       
                        @endforeach    
                    </tbody>
                </table>
...
@endsection
	...
	<script>
     ...
     $('select[name="zone"]').on('change', function() {
            ...
            $('#table2').DataTable().column(3).search('^'+stateID,true,false).draw();
     });
    ...
    $(document).on('click', '.add-modal', function() {
       if($('.checkbox:checked').length==0){
            ...
            $('.modal-title').text('ስርርዕ ዋህዮ መመዝገቢ ቕጥዒ');
            $('.deleteContent').hide();
            $('.form-horizontal').show();
            ...
            $('.checkbox:checked').each(function(){
                // if(this.checked){
                rows.push($(this).parent().parent().children()[8]);
                  var myVal=$(this).val();
                  idVals.push(myVal);
                // }
            });
