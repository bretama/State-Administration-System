...
ወሰኽን ጉድለትን ኣሃዛዊ መረዳእታ ላዕለዋይ ኣመራርሓ
@endsection
...
-> where('membershipType', '=', 'ላዕለዋይ ኣመራርሓ')->count(); 
    echo $count?></td>
    <td ><?php $now= carbon::today(); $then=$now->subMonths(3); $count1 = DB::table('approved_hitsuys')->join('siltenas', 'approved_hitsuys.hitsuyID', '=', 'siltenas.hitsuyID') 
    ...
    ->count(); 
    echo $count1?></td>
<td ><?php $now= carbon::today(); $then=$now->subMonths(3); $count2 = DB::table('approved_hitsuys')->join('midebas', 'approved_hitsuys.hitsuyID', '=', 'midebas.hitsuyID') 
    ->where('deraja', '=', 'ላዕለዋይ ኣመራርሓ') 
    ->whereBetween('midebas.created_at', [$now,  $then])
    ->where('approved_hitsuys.hitsuyID','LIKE','0'.$i.'%')
    ->count();
    $count3 = DB::table('approved_hitsuys')->join('transfers', 'approved_hitsuys.hitsuyID', '=', 'transfers.hitsuyID') 
    ->where('dereja', '=', 'ላዕለዋይ ኣመራርሓ') 
    ->whereBetween('transfers.created_at', [$now,  $then])
    ->where('approved_hitsuys.hitsuyID','LIKE','0'.$i.'%')
    ->count(); 
    echo $count2 + $count3 ?></td>
...
->count(); 
    echo $edu5?></td>
<td ><?php $edu6= DB::table('approved_hitsuys')->join('transfers', 'approved_hitsuys.hitsuyID', '=', 'transfers.hitsuyID')
-> where('membershipType', '=', 'ላዕለዋይ ኣመራርሓ')
->where('transfers.reason', '=', 'timhrti') 
->whereBetween('transfers.created_at', [$now,  $then])
->where('approved_hitsuys.hitsuyID','LIKE','0'.$i.'%')
->count(); 
    echo $edu6?></td>
    <td ><?php $edu7= DB::table('approved_hitsuys')->join('transfers', 'approved_hitsuys.hitsuyID', '=', 'transfers.hitsuyID')
-> where('membershipType', '=', 'ላዕለዋይ ኣመራርሓ')
->where('transfers.reason', '=', 'timhrti') 
->whereBetween('transfers.created_at', [$now,  $then])
->where('approved_hitsuys.hitsuyID','LIKE','0'.$i.'%')
->where('transfers.place','Not LIKE','0'.$i.'%')
->count(); 
    echo $edu7?></td>
...
<td></td>
<td ><?php $count = DB::table('approved_hitsuys')-> where('membershipType', '=', 'ላዕለዋይ ኣመራርሓ')->count(); 
    echo $count?></td>
</tr>
...
<div>
      <a href="{{ route('HtmlToPDF2') }}" class="btn btn-primary">Download in PDF</a>
  </div>
</br>
      </div>
    </div>
