      ...
      <table class="table table-borderless" id="table2">
        ...
        <tbody  id="products-list" name="products-list">
          ...
          <tr class="item{{$item->id}}" >
            <td>{{$item->id}}</td>
            <td>{{$zobadatas[$woredazone[$item->woredacode]]}}</td>
            <td>{{$woredadatas[$item->woredacode]}}</td>
            <td>{{$item->planyear}}</td>
            <td>{{$item->plantype}}</td>
            <td>{{$item->amount}}</td>           
            <td>{{$item->descrpt}}</td>            
          ...
          <input type="hidden" id="fid">
            <!-- <div class="form-group">
              ...
              <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                  <select class="form-control" name="woreda" id="woredacode1" required="required">
  ...
  <script>
   ...
   var row, stuff;
    $(document).on('click', '.edit-modal', function() {
        ...
        stuff = [$(row[0]).html(),$(row[1]).html(),$(row[2]).html(),$(row[3]).html(),$(row[4]).html(),$(row[5]).html(),$(row[6]).html()];
        fillmodalData(stuff)
        $('#myModal').modal('show');
    });
...
function fillmodalData(details){
    ...
    $('#planyear1').val(details[3]);
    $('#plantype1').val(details[4]);
    $('#amount1').val(details[5]);
    $('#descrpt1').val(details[6]);
        ...
        $.ajax({
            ...
            success: function(data) {
              if(data[0] == true){
                      ...
                      $(row[3]).html($('#planyear1').val());
                      $(row[4]).html($('#plantype1').val());
                      $(row[5]).html($('#amount1').val());
                      $(row[6]).html($('#descrpt1').val());
