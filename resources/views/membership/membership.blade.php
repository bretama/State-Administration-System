					...
					<table class="table table-hover" id="table2">
						...
						<tbody>
							...
							<tr style="background-color: {{ $bg_color }}">
								...
								<td>{{ $mydata->name }} {{ $mydata->fname }} </td>
								<td>{{ $mydata->gender }}</td>
								...
								<td><button class="add-modal btn btn-success btn-xs" data-info="{{ $mydata->hitsuyID }},{{ $mydata->name }} {{ $mydata->fname }} {{ $mydata->gfname }},{{ $mydata->tabiaID }}" title="ኣባልነት ይፅደቕ">
									<span class="fa fa-check-circle"></span>ኣባልነት ይፅደቕ</button>
									@if($mydata->hitsuy_status=='ሕፁይ')
									<button class="edit-modal btn btn-success btn-xs" data-info="{{ $mydata->hitsuyID }},{{ $mydata->name }} {{ $mydata->fname }} {{ $mydata->gfname }},{{ $mydata->tabiaID }}" title="ይናዋሕ">
									<span class="fa fa-plus-circle"></span>ይናዋሕ</button>
									@endif
                                    @if($mydata->hitsuy_status!='ሕፁይነት ተሰሪዙ')
									<button class="delete-modal btn btn-warning btn-xs" data-info="{{ $mydata->hitsuyID }},{{ $mydata->name }} {{ $mydata->fname }} {{ $mydata->gfname }},{{ $mydata->tabiaID }}" title="ይሰረዝ">
