...
@section('main-content')
   <div class=""> 
        ...
        <form method="GET" action= "{{URL('taramemberslist')}}" class="form-inline">                           
            <label>ቕድሚ ኸዚ ዝተመዝገቡ ናይ ተራ ኣባላት ህወሓት መምልኢ ማህደር:</label>
        ...
        <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
                          {{ csrf_field() }}
                    <div class="form-group col-sm-12 col-md-12">                     
                        <label class="col-md-3 col-sm-3 col-xs-3" for="hitsuyID">መፍለይ ቑፅሪ ተራ ኣባል:</label>
                    ...
                    <div class="form-group col-sm-12 col-md-12">                     
                        <label class="col-md-12 col-sm-12 col-xs-12" for="remark">3. ናይ በዓል ዋና ሪኢቶ</label>
                    ...
                    <label class="col-md-3 col-sm-3 col-xs-3"> </label>
                    <div class="col-md-3 col-sm-3 col-xs-3">
                        <button type="button" id="saveRecord" class="btn btn-success">መዝግብ</button>
                    </div>
                    <div>
                        <input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
                    <p style="text-align:center">***ናይ ተራ ኣባላት ህወሓት መምልኢ ማህደር***</p>
                    </div>
                    </div>
                </form>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">       
                        ...
                        <div class="form-group col-md-12 col-sm-12 col-xs-12">                        
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                  <select class="form-control" id="members" name="members" required="required">
                                        <option selected disabled>~ተራ ኣባል ምረፅ~</option>
                                    </select>
                              </div>                                
                        </div>
                    ...
                    <div class="deleteContent">
                        መፍለይ ቑፅሩ "<span class="hID text-danger"></span>" ዝኾነ ተራ ኣባል ብትክክል ክጠፍአ ይድለ ድዩ  ? <span
     ...
     $('#saveRecord').on('click', function(){ 
                                       $.ajax({
                                        ...
                                        success: function(data) {
                                              ...
                                              $('input[name="evaluation"]:checked').prop('checked',false);
...
$(document).on('click', '.search-modal', function() {
        ...
        $('.modal-title').text('ተራ ኣባል ካብ ማህደር መድለይ');
        $('.deleteContent').hide();
        $('.searchContent').show();
        $('.formadder').hide();
        $('#myModal').modal('show');
    ...
    $('select[name="zone"]').on('change', function() {
        ...
        $('select[name="proposerWahio"]').on('change', function() {
            var stateID = $(this).val();
               ...
               if(stateID) {
                $.ajax({
                    url: 'myform2/ajax/taramember/'+stateID,
                    type: "GET",
                    dataType: "json",
                    success:function(data) {
                        ...
                        $('select[name="members"]').append('<option value="'+ " " +'">'+ "~ተራ ኣባል ምረፅ~" +'</option>');
                        $.each(data, function(key, value) {
