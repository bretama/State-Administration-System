		  	...
		  	<div style="padding: 0px 0px 10px 385px">
		    	<button class="btn search-modal"><span class="glyphicon glyphicon-search"></span> ካብ ማህደር ድለ</button>	
						...
						<div class="form-group">
							...
							<div class="col-md-3 col-sm-6 col-xs-12">
								<input type="text" id="startDate1" name="startDate" required="required" class="form-control col-md-7 col-xs-12" value="{{ $cnt ? Request::old('startDate') : '' }}">
							</div>
						</div>
					  <br/>
						<br/>
				...
				<div class="table-responsive text-center">
					<table class="table table-striped table-borderless" id="table2">
						...
						<tbody>
							...
							<tr>
								<input type="hidden" value="{{ $mypen->id }}">
								...
								<td>{{ App\DateConvert::toEthiopian(date('d/m/Y',strtotime($mypen->startDate))) }}</td>
								<td>
									@if (array_search(Auth::user()->usertype, ['admin', 'zoneadmin', 'woredaadmin']) !== false)
									<button class="edit-modal btn btn-info" data-info="{{$mypen->hitsuyID}},{{ $mypen->chargeType }},{{ $mypen->chargeLevel }},{{ $mypen->penaltyGiven }},{{ $mypen->proposedBy }},{{ $mypen->approvedBy }},{{ $mypen->duration }},{{ $mypen->startDate }},{{$mypen->id}}">
